...
#
# Tags look like `v<InfluxDB version>-<release>`:
#
# - if defaults/main.yml points at an InfluxDB version that has never been
#   released, the release counter restarts at 0 (`v2.9.2-0`)
# - otherwise the counter is incremented (`v2.9.2-1`), but only if something
...
# `influxdb_version` is the only place in defaults/main.yml that names a
# version literally; everything else (`influxdb_container_image_tag`,
# `influxdb_container_image`) derives from it through Jinja and would come out
# of this file as an unrendered `{{ ... }}` string. Anchoring on the start of
# the line and requiring the colon immediately after the name is what keeps
# lookalikes (a commented-out version, an `influxdb_version_*` variable) from
# being picked up instead.
version="$(sed -nE 's|^influxdb_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the InfluxDB version from $defaults_path"
	exit 1
fi
